...
Only if we have `hidden variables`...
   ...
   - *In VI, the **KL-Divergence** (where each `q(z)` is a finger) should be **`minimized`**. 
   - *In VAE, the `Jansen's lower bound` as a **KL-Divergence** needs to be **`maximized`**..(where `q(z)` is a mixing coefficient for GMM form with `log(p/q)` as a Gaussian cluster) and each Jansen's lower bound is a finger. So..in VI, the **latent variable estimator** is the joint of all `q(z)`, while in VAE, the **latent variable estimator** is the sum of a bunch of `Jansen's lower bound`ssss. 
